...
function generateConversionCompileMethod() {
  let out = `
// Method auto generated by script
// Because this was too tedious to write by my own
//   - Jules
void compileConversionOp(
  BytecodeWriter* writer, 
  primitivekind from, primitivekind to, 
  registerid in, registerid out
) {
  switch (from) {`
  ...
  for (const fromtype of NUMBER_TYPES) {
    ...
    for (const totype of NUMBER_TYPES) {
      ...
      out += `\n        case PK_${totype.fullname.toUpperCase()}:`
      out += `\n          writer->appendOpCode(OP_${opcode});`
      out += `\n          writer->appendU8(in);`
      out += `\n          writer->appendU8(out);`
      out += `\n          writer->appendPadding(PAD_${opcode});`
      out += `\n          return;`
    }
    out += `\n        default:`
    out += `\n          return;`
