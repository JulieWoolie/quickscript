...
#include "qs/codegen/CompilerContext.hpp"
...
static void compileIdentifier(
  ...
  if (sym->stype() == SYM_LocalVar) {
    const LocalVarSymbol* lvs = static_cast<LocalVarSymbol*>(sym);
...
static void compileCallExpr(const CallExpr* call, const RegisterId out, CompilerContext& ctx) {
  ...
  for (uint32 i = argCount; i > 0; i--) {
    ...
    const ScriptType* argType = targetSign->getArgumentType(argIdx);
    const uint64 argSize = argType->stackSizeBytes();

    runningOffset -= argSize;
    offsets[argIdx] = runningOffset;
  }
