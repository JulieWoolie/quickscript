...
int32 VirtualMachine::beginExecution(const uint32 funcEntryIdx, const std::vector<std::string_view>& args) {
  ...
  if (args.size() == 0) {
    ...
    QsArray arr = m_heap.allocConstArray(args.size(), POINTER_SIZE);
    argsAddr = arr.address();

    for (uint32 i = 0; i < args.size(); i++) {
      const std::string_view& arg = args[i];

      QsArray str = m_heap.allocConstString(arg.length());
      memcpy(str.data, arg.data(), arg.length());
